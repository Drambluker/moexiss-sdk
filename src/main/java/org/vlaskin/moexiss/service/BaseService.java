...
public abstract class BaseService
{
    ...
    protected String get(CharSequence request) throws IOException
    {
        ...
        log.debug("Request: {}", toSafeRequestTarget(requestUrl));
        return httpTransport.get(requestUrl);
    }
    ...
    {
        String normalizedUrl = removeTrailingSlash(
                Validate.notBlank(baseUrl, "Base URL must not be blank").strip());
        Validate.notBlank(normalizedUrl,
        ...
        if (!("http".equalsIgnoreCase(uri.getScheme())
            ...
            throw new IllegalArgumentException("Base URL must be an absolute HTTP(S) URL");
        if (uri.getRawUserInfo() != null || uri.getRawQuery() != null || uri.getRawFragment() != null)
            throw new IllegalArgumentException(
                    "Base URL must not contain user info, query, or fragment");
        return normalizedUrl;
    }

    protected static String toSafeRequestTarget(String requestUrl)
    {
        URI uri = URI.create(requestUrl);
        StringBuilder target = new StringBuilder(uri.getScheme()).append("://");
